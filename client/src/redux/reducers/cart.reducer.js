...
const cartReducer = (state = initialState, { type, payload }) => {
  switch (type) {
    ...
    case 'ADD_TO_CART':
      ...
      const includedIndex = state.products.findIndex(
    ...
    case 'REMOVE_ONE':
      ...
      localStorage.setItem(
        'totalQuantity',
        JSON.stringify(state.totalQuantity - 1)
      )
    ...
    case 'REMOVE_MANY':
      const removedManyItem = state.products.filter(
        item => item._id === payload
      )[0]

      const reducedItems = [
        ...state.products.filter(item => item._id !== payload)
      ]
      const reducedQuantity = state.totalQuantity - removedManyItem.quantity

      const reducedTotalPrice = round(
        state.totalPrice - removedManyItem.price * removedManyItem.quantity,
        2
      )
    ...
    case 'UPDATE_QUANTITY': {
      ...
      const oldProductTotalPrice = round(
        oldProduct.price * oldProduct.quantity,
        2
      )
      ...
      return {
        ...
        totalQuantity:
          state.totalQuantity - oldProductQuantity + newProductQuantity,
